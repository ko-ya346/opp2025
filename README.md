...
## 8/17
- cv の切り方を工夫するも cv スコア改善して lb は改善せず
  - 各 fold は元データだけで切る、追加データは常にフル使用
  - cv を信じたほうがよいか？
- gnn で desc 使えてなかったので改修、スコアちょっと良い
- gnn で トリマー環状物使えてなかったので修正、ちょっといい
- graph pooling = mean、sum よりもよい
- 上3つ全部乗せするも、FFV が微妙に良化したのみ
- chatgpt に主鎖を取り出すスクリプト書いてもらった
...
## 8/19
- アンサンブルを試してみたが、wandb のスコアと oof のスコアが一致しない。id じゃなくて SMILES で突合したほうがいいかも
  - exp038 で検証
  - 外部データのありなしで大きく mae が変わる。特に Tg
  - 追加データの分離で外部データによる補完を考慮していなかった
- autotime 使うと tqdm プログレスバーが表示されない
  - 修正したい
- gnn の 実験しなおし exp037, 各条件を 3かいずつ回す
  - desc 追加、graph pooling=mean は効果ありそう

### next action
- fold の切り方改善
  - target 毎に都度計算するのではなく、まとめて計算（再現性◎）-> null 多いターゲットだと困るのでやっぱやらない
  - 外部データ、外部補完データの扱い
  - 外部データで補完すると分離が面倒なので、単純に追加したほうがいいかもしれない

## 8/20
- lgb で 外部データの追加方法と fold の切り方修正してみる
  - 外部データはすべて union で結合、補完しない
  - 外部データにはフラグつける
  - cv の時、valid は元データのみ使う
  - fold はすべてのデータで分割
  - valid はオリジナルデータのみ
...
## 8/25
- public LB に惑わされないようにしよう（テストデータの 8%程度）
- 公開Code との乖離調査
  - https://www.kaggle.com/code/guanyuzhen/lb-0-064-neurlps-2025-baseline-random
  - Code をコピペしてモデルを LGBMClassifier にしただけで提出したところ LB 0.070
  - LB 良化している原因はモデルのハイパラチューニング！特徴量生成や前処理などの差分ではなかった！
  - 自分のコードに重大な欠陥がないことが示されたのでとてもよかった
- exp045 で cv 戦略整理
  - valid データには元のデータのみ（外部データで補完した値も含まない）
  - valid に含まれる smiles は train から除外
  - fold は scaffold で分割して固定化
  - 目的変数毎に各 fold のボリュームを確認し、3分割であればある程度ボリューム確保できていた
  - mae や CV スコア計算も元データのみで実施

### next action

- lgb (目標 8/28まで)
  - 特徴量削減スクリプト（各 fold の train をさらに分割して上位 N % の特徴量を選抜）
  - 3D 特徴量追加 -> exp046
  - ハイパラ探索の実装
- GNN (目標 9/1 まで)
  - cv 修正して再学習 -> exp047-1
  - 結合点が 2 つ以外の smiles 除外
  - 三量体環状物の再学習 -> exp047-2
  - 特徴量有無で再学習 -> exp047-3
- アンサンブル
  - アンサンブルの戦略整理
  - アンサンブルのパイプライン実装
  - 提出用 notebook をアンサンブル対応

## 8/26
- アンサンブルしたら wMAE 0.056（check_ensemble）
- gnn で fold 分割して学習させたときの、平均エポックを記録したい
  - modeling_gnn.py を修正中
  - 保存したモデルに記録されてた（model.fitting_loss の要素数）

## 8/27
- train_gnn.py 整理
- アスタリスク2個以外除外, トリマー三量体失敗した行を除去
  - 一長一短という感じ。多様性が増えたということにしよう
  - 学習前に行を除去してしまい、アンサンブルしづらくなったので再学習
...
- train_gnn.py 修正
- exp049修正（lgb optuna）
- 行数が異なる場合もアンサンブルできるように修正
  - 一番スコアがいいモデルの重み、意外と高くない
  - GNN 系がちゃんと貢献していて嬉しい
  - wMAE 0.056 出てる。単一モデルではここまでのスコア出ない 
- そろそろ提出していきたい

### next action
- アンサンブルで重み計算して保存するスクリプト
- exp049 実行
- gnn 系全データで学習
- アンサンブルして提出するパイプライン構築

## 8/31
- exp048-3 提出 cv: 0.057, lb: 0.066 思ったより良くない
- アンサンブルする notebook 作った
- アンサンブルするスクリプト書いた
- 主鎖の smiles の情報を使う実装 exp050
- optuna でチューニングしたけどイマイチスコア上がらなかった。なんでや

### next action
- 置換基の情報追加
- アンサンブルしたモデルを提出
- 末端の処理
- train, valid の分け方（tanimoto 類似度で似ているものを除外）

## 9/1
- 主鎖の情報追加した
  - cv: 0.053, lb: 0.091
  - 過学習を疑い、tanimoto 類似度でフィルタを試みたがそんなに類似度高いものはなかった
  - 特徴量生成時にリネームしているが、あの辺で特徴量名が非決定的で学習時の条件とずれている可能性がある
  - exp051 で修正
- desc など特徴量生成時に * あると分布変わる？
  - maccs, desc で確認したが結構異なる結果になった
  - どちらがよいか確認しよう
- 主鎖取り出すのに失敗した場合の処理
  - 空文字入れたらカーネル落ちた
  - test データが失敗することを考慮にいれる必要があるぞ

## 9/2
- アンサンブルの lb めっちゃ悪い。闇雲に精度悪いモデルを入れるのは逆効果
  - lgb モデルの mae をベースにして、近しい値のものだけ採用
- 主鎖の情報、失敗したら空にする